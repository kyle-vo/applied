...
function ScoreTooltip({ active, payload }) {
  ...
  const { label, sublabel, count } = payload[0]?.payload || {};
  return (
...
export default function Analytics() {
  ...
  const funnelData = useMemo(() => {
    ...
    return ["applied", "screening", "interview", "offer"].map((key) => ({
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          accentClass={summary?.response_rate >= 20 ? "text-emerald-600" : undefined}
        />
        <StatCard
          label="Avg AI match"
